...
let spawnPlayer () : Entity = 
    let player = Player (Vector2(100.0f,0.0f), 10.0f)
    player :> Entity


let updateEntites (ks:KeyboardState, ms:MouseState, dt:float32, entities:List<Entity>) =
    
    let entities = 
        match entities with
        | Empty -> spawnPlayer() << entities
        | Node(entity,list) ->
            entity.UpdateState(ks, ms, dt, entity) << list
    entities
...
let drawEntity (entity:Entity) : Drawable = {
    Drawable.Position = entity.Position
    Drawable.Image = entity.Image
}
...
let initialState() = 
    {
        Player      = Player(Vector2(100.0f,100.0f), 20.0f)
        Entities    = Empty
...
let updateState (ks:KeyboardState) (ms:MouseState) (dt:float32) (gameState:GameState) =
  ...
  { 
    gameState with Ship         = updateShip(ks, ms, dt, gameState.Ship)
                   ...
                   Gun          = newGun
                   Entities     = updateEntites(ks, ms, dt, gameState.Entities)
  }
...
let drawState (gameState:GameState) : seq<Drawable> =
    let listOfEntities = 
        map drawEntity gameState.Entities |> toFSharpList
    ...
    ] @ listOfDrawableEnemies @ listOfDrawableProjectiles @ listOfEntities
        |> Seq.ofList
